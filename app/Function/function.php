...
function show_cat_parent($arr , $parent = 0 ,$str = "--"){
	foreach($arr as $item){
		$id = $item['id'];
		$name = $item['name'];
		...
		if($item['parent_id'] == $parent){
		 ...
		 show_cat_parent($arr,$id,$str."--");
	}
}

// Hàm xuất thông tin về level admin
...
//    key => hiển thị cho người dùng
    ...
    if (array_key_exists($level, $list_users)) {
        return $list_users[$level];
    }
}
